...
router.post("/items", async (req, res) => {
  try {
    const { nm_item, dt_criacao, dt_ultima_modificacao, fl_status, id_lista } =
      req.body;

    const result = await db_query(
      "INSERT INTO tb_item (nm_item, dt_criacao, dt_ultima_modificacao, fl_status, id_lista) VALUES (?, ?, ?, ?, ?)",
      [
        nm_item,
        new Date(dt_criacao),
        new Date(dt_ultima_modificacao),
        fl_status,
        id_lista,
      ]
    );
...
router.get("/items", async (req, res) => {
  try {
    const items = await db_query(
      "SELECT id_item, nm_item, dt_criacao, dt_ultima_modificacao, fl_status, id_lista FROM tb_item"
    );
    res.json(items);
  } catch (err) {
...
router.put("/items/:id", async (req, res) => {
  try {
    ...
    await db_query(
      "UPDATE tb_item SET nm_item = ?, dt_criacao = ?, dt_ultima_modificacao = ?, fl_status = ?, id_lista = ? WHERE id_item = ?",
      [
        nm_item,
        new Date(dt_criacao),
        new Date(dt_ultima_modificacao),
...
router.patch("/items/:id", async (req, res) => {
  try {
    ...
    await db_query(`UPDATE tb_item SET ${setQuery} WHERE id_item = ?`, [
      ...values,
      itemId,
    ]);
    ...
    res.sendStatus(500).send("Erro ao atualizar parcialmente o item");
  }
});
